...
_CacheCursor.getSavedCacheCursorPath = (userId) => {
  ...
  return {
    PHOTOS_CACHE_CURSOR_PATH: path2.resolve(baseDir, "photos.json"),
    REELS_CACHE_CURSOR_PATH: path2.resolve(baseDir, "reels.json"),
    VIDEOS_CACHE_CURSOR_PATH: path2.resolve(baseDir, "videos.json")
  };
};
...
PathUtils.getSavedUserMediaDirPath = (userId) => {
  ...
  return {
    PHOTOS_SAVED_DIR: path4.resolve(BASE_DIR, "photos"),
    REELS_SAVED_DIR: path4.resolve(BASE_DIR, "reels"),
    VIDEOS_SAVED_DIR: path4.resolve(BASE_DIR, "videos"),
    STORIES_SAVED_DIR: path4.resolve(BASE_DIR, "stories")
  };
};
...
\u{1F680} Start downloading photos of user ${userId}...`);
      ...
      await DownloadUtils_default.downloadByBatch(
        ...
        async (photo, index) => {
          ...
          const savedFilePath = path5.resolve(PHOTOS_SAVED_DIR, fileName);
...
\u{1F680} Start downloading reels of user ${userId}...`);
      ...
      await DownloadUtils_default.downloadByBatch(
        ...
        async (reel, index) => {
          ...
          const savedFilePath = path6.resolve(REELS_SAVED_DIR, fileName);
      ...
      await DownloadUtils_default.downloadByBatch(
        ...
        async (storyMedia, index) => {
          ...
          await DownloadUtils_default.downloadMedia(
            storyMedia.uri,
            path7.resolve(baseDir, fileName)
          );
        },
        true
      );
...
\u{1F680} Start downloading videos of user ${userId}...`);
      ...
      await DownloadUtils_default.downloadByBatch(
        ...
        async (video, index) => {
          ...
          const savedFilePath = path8.resolve(VIDEOS_SAVED_DIR, fileName);
...
export {
  FacebookDownloader_default as FacebookDownloader
};
